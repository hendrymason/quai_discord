...
signUpDict['Spartans'] = signUpIntro + "\n\n" + "To be rewarded for running a Node or Miner on Quai Network as a SPARTAN: " + spartanForm
    ...
    if role is not None:
      ...
      if member is not None:
        ...
        if inList == False:
          ...
          await channel.send(content=completeSignUp, delete_after=30.00, mention_author=True)
        ## when user inList for selected role
          ...
          await channel.send(content=f"Congratulations {member.mention}! you've been added as part of the "+ str(role.name).upper() + " for the Bronze Age Testnet!", mention_author=True)
        ## when user inList, but not in role specific list
          ...
          await channel.send(content=signUpDict[role.name], delete_after=30.00, mention_author=True)
